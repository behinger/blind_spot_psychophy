...
dat = subset(allDat,allDat$removeReason!='Single')
dat.ortho = subset(dat,dat$success==1&dat$stimPVisible==2)
dat.uni = subset(dat,dat$success==1&dat$stimPVisible!=2)

perf.ortho = ddply(dat.ortho,.(subject,stimRLoc,stimLLoc),summarise,perf=mean(correct))
...
taskInverse = ddply(perf.ortho,.(subject),summarise,num = sum(perf<0.5))
taskInverse.vp = taskInverse$subject[taskInverse$num>2]

taskBad = ddply(perf.ortho,.(subject),summarise,num = sum(perf<0.75))
...
if(Nremaining!=length(unique(allDat$subject[allDat$remove==F]))){warning('something does not add up!')}
...
Ntrialsremaining = ddply(dat.uni,.(subject),summarise,nT=length(trial))$nT
trialsQuantile = round(quantile(Ntrialsremaining,c(0.05,0.95)))

```

After cleaning trials for fixation deviation and blinks on average `r round(mean(Ntrialsremaining),0) ` [90%-quantile:`r trialsQuantile[1]`, `r trialsQuantile[2]`] remained.
